...
import { MatDialog, MatSnackBar, MatPaginator, MatSort, MatDatepickerInputEvent } from '@angular/material';
...
export class ListstatusComponent implements OnInit {
  ...
  displayedColumns = [ 'userId', 'user','userIntime','userDate','userMobileNumber','userHostname','userCity','userPurpose'  ];
  dataSource: any=[];
...
ChangeDateFormat(){
   ...
   if ((typeof date) == 'object') {
    ...
    this.Currentdate = splitDate[2] + '-' + splitDate[1]+'-'+ splitDate[0]
           ...
           this.appointmentService.getAppointListtByDateStatus(1,this.Currentdate).then(result=>{
